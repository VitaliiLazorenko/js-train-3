...
function stringToNumber(str) {
  ...
  return Number(str); // Повертаємо отримане число
}
...
function isAlmostSame(num1, num2) {
  return Math.abs(num1 - num2) < Number.EPSILON; // Визначаємо, чи є різниця між двома числами меншою або рівною EPSILON
}
...
function isSafeInteger(num) {
  return Number.isSafeInteger(num); // Перевіряємо, чи є число меншим або рівним MAX_SAFE_INTEGER і більшим або рівним MIN_SAFE_INTEGER
}
...
function isTooLarge(num) {
  return num > Number.MAX_VALUE || num < -Number.MAX_VALUE; // Перевіряємо, чи є число більшим за MAX_VALUE
}
...
function isAlmostZero(num) {
  return num > Number.MAX_VALUE && num < Number.MIN_VALUE; // Перевіряємо, чи є число більше 0, але все ще менше за Number.MIN_VALUE
}
